...
describe('MemoryService Integration', () => {
    ...
    it('should store and retrieve memories via semantic search', async () => {
        ...
        const stored = await memoryService.storeMemory(content, metadata, 1.0, 1.0);
        expect(stored.content).toBe(content);
        expect(stored.id).not.toBe("error");

        const retrieved = await memoryService.findSimilarMemories("core vibration", 1);
    ...
    it('should handle memory decay', async () => {
        ...
        const retrieved = await memoryService.findSimilarMemories(content, 10);
    ...
    it('should prune memories below threshold', async () => {
        ...
        await memoryService.storeMemory(content, {}, 0.1, 0.05); // Already at threshold

        await memoryService.decayMemories(1.0, 0.1); // Small decay to trigger pruning
