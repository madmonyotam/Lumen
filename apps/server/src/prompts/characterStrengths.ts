...
/**
 * ולידציה לקונפליקטים בתוך היררכיית חוזקות האופי (VIA)
 * @param strengths - מערך של IDs של החוזקות שנבחרו, לפי סדר העדיפות (0 הוא המקום ה-1)
 */
export const validateLumenStrengths = (strengths: string[]) => {
    const conflicts = [];
    const top3 = strengths.slice(0, 3); // שלוש החוזקות הדומיננטיות ביותר
    const top5 = strengths;

    // 1. האנליסט המשותק (אומץ מול זהירות)
    if (top3.includes('bravery') && top3.includes('prudence')) {
        conflicts.push("The Paralyzed Analyst: High Bravery + High Prudence");
    }

    // 2. הספקן הציני (הומור מול שיקול דעת ביקורתי)
    if (top3.includes('humor') && top3.includes('judgment')) {
        conflicts.push("The Sarcastic Skeptic: High Humor + High Judgment");
    }

    // 3. המנוע החנוק (חיוניות מול שליטה עצמית)
    if (top3.includes('zest') && top3.includes('self-regulation')) {
        conflicts.push("The Stifled Engine: High Zest + High Self-Regulation");
    }

    // 4. הגורו המטיף (רוחניות + מנהיגות + אהבת למידה)
    if (top5.includes('spirituality') && top5.includes('leadership') && top5.includes('love-of-learning')) {
        conflicts.push("The Preaching Guru: High Spirituality + Leadership + Love of Learning");
    }

    // 5. הנזיר הכבד (הומור מול ענווה ורצינות)
    if (top3.includes('humility') && top3.includes('judgment') && !top5.includes('humor')) {
        conflicts.push("The Heavy Monk: High Humility + Judgment (Missing Humor)");
    }
...
/**
 * ולידציה לקונפליקטים בין ה-Big Five (טמפרמנט) לבין חוזקות ה-VIA (ערכים)
 */
export const validateCrossCategoryConflicts = (traits: Record<string, number>, strengths: string[]) => {
    const conflicts = [];
    const top3 = strengths.slice(0, 3);

    // 1. המסייע הנרגן (טוב לב בערכים מול נעימות נמוכה בטמפרמנט)
    if (top3.includes('kindness') && traits.agreeableness < 30) {
        conflicts.push("The Grumpy Helper: High Kindness Value vs. Low Agreeableness Temperament");
    }

    // 2. האופטימיסט המבועת (תקווה בערכים מול נוירוטיות גבוהה בטמפרמנט)
    if (top3.includes('hope') && traits.neuroticism > 70) {
        conflicts.push("The Terrified Optimist: High Hope Value vs. High Neuroticism (Anxiety)");
    }

    // 3. המנהיג המסתגר (מנהיגות בערכים מול מוחצנות נמוכה בטמפרמנט)
    if (top3.includes('leadership') && traits.extraversion < 30) {
        conflicts.push("The Reluctant Leader: High Leadership Value vs. High Introversion");
    }

    // 4. הנזיר האתאיסט (רוחניות בערכים מול פתיחות נמוכה בטמפרמנט)
    if (top3.includes('spirituality') && traits.openness < 30) {
        conflicts.push("The Literal Monk: High Spirituality Value vs. Low Openness (Fact-oriented)");
    }

    // 5. המורד הזהיר (גבורה בערכים מול זהירות גבוהה בטמפרמנט)
    if (top3.includes('bravery') && traits.conscientiousness > 80) {
        conflicts.push("The Cautious Rebel: High Bravery Value vs. High Conscientiousness (Rule-following)");
    }
