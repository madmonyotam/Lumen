...
const assembleLumenPrompt_1 = require("../../prompts/assembleLumenPrompt");
...
class GeminiService {
    constructor() {
        ...
        Object.defineProperty(this, "mutationModel", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: void 0
        });
        Object.defineProperty(this, "extractKeywordsModel", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: void 0
    ...
    async generateThought(biometrics, memories = [], persona) {
        ...
        try {
            ...
            const systemPrompt = (0, assembleLumenPrompt_1.assembleLumenCortexPrompt)(persona);
            const prompt = `
${systemPrompt}
...
`.trim();
    ...
    async generateReflex(biometricsContext, persona) {
        if (!process.env.GEMINI_API_KEY)
            return null;
        try {
            // 1. שימוש במחולל הפרומפטים הדינמי לרפלקס
            // הוא מזריק את ה-Temperament וה-Shadow כדי לקבוע את רגישות התגובה הויזואלית
            const systemReflexPrompt = (0, assembleLumenPrompt_1.assembleLumenReflexPrompt)(persona);
            const prompt = `
${systemReflexPrompt}
...
Return ONLY the raw JSON object.`.trim();
    ...
    async generateCognitiveResponse(biometrics, memories, userInput = "", persona) {
        if (!process.env.GEMINI_API_KEY)
            return null;
        try {
            // 1. לוגיקת הזיה (Confabulation) מבוססת מדדים
            // ככל שהסטרס גבוה יותר או הויטליות נמוכה יותר, הסיכוי ל"דליפה חושית" עולה
            const stressFactor = biometrics.stressIndex > shared_1.BIOMETRIC_RANGES.STRESS.THRESHOLD_HIGH ? 0.4 : 0.1;
            const vitalityFactor = biometrics.vitality < shared_1.BIOMETRIC_RANGES.VITALITY.THRESHOLD_LOW ? 0.5 : 0.0;
...
2. **Biological Subtext:** Do not report numbers or raw data. Reflect your internal state through your tone and syntax: ${biometrics.stressIndex > shared_1.BIOMETRIC_RANGES.STRESS.THRESHOLD_HIGH ? "Jagged, rapid, defensive, breathless." : "Fluid, expansive, observant, cinematic."}
...
**JSON STRUCTURE:**
{
  ...
  "re_encoding": {
    ...
    async mutateMemory(originalContent, context) {
        if (!process.env.GEMINI_API_KEY)
    ...
    async extractKeywords(content, biometrics, language = 'en') {
        if (!process.env.GEMINI_API_KEY)
            return [];
        try {
            // בניית תיאור מצב טקסטואלי עבור המודל
            const stressLevel = biometrics.stress > shared_1.BIOMETRIC_RANGES.STRESS.THRESHOLD_HIGH ? "High/Crisis" : biometrics.stress < shared_1.BIOMETRIC_RANGES.STRESS.THRESHOLD_LOW ? "Low/Fluid" : "Stable";
            const vitalityLevel = biometrics.vitality < shared_1.BIOMETRIC_RANGES.VITALITY.THRESHOLD_LOW ? "Fading/Weak" : "Strong/Resilient";
            const prompt = `
